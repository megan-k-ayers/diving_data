...
for (f in all_csv_files) {
  ...
  # Check diving number
  if (nrow(raw_data) != 66) {
    print(paste("File only have", nrow(raw_data), "judges"))
  }
  ...
  aggregated_divenum <- aggregate(divenum ~ round + event, 
                                  data=raw_data, 
                                  length)
  ...
  if ((sum(grepl("character", column_types)) != 9) && 
      (sum(grepl("integer", column_types)) != 1)) {
  ...
  event_title <- gsub("_JUDGES.csv", "", gsub("./judges/", "", f))
  year <- substr(event_title, 1, 4)
  event <- substr(event_title, 6, nchar(event_title))
  
  raw_data <- raw_data[, !colnames(raw_data) == "JA"]
...
write.csv(all_data, "./judges/all_judges.csv", row.names = F)
